 ...
 */
const writeToCsvFile = async (outputPath, { requestUrl, referenceUrl, statusCode }) => {
    let writer = fs.existsSync(outputPath)
        ? csvWriter({ sendHeaders: false })
        : csvWriter({ headers: ["referenceUrl", "requestUrl", "statusCode"]});

    writer.pipe(fs.createWriteStream(outputPath, { flags: 'a' }));
    writer.write({ referenceUrl, requestUrl, statusCode });
    writer.end();
};
...
const brokenLinksObserver = new Proxy(brokenLinks, {
    set: function(target, key, value) {
        ...
        // Write to CSV
        config.output && writeToCsvFile(config.output, value);
    }
} );
